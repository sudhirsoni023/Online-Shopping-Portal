...
<%
	...
	ResultSet rs1 = st.executeQuery("Select * from users inner join cart on cart.email=users.email and cart.status='bill'");
	while(rs1.next())
	{
...
<div class="right-div-right"><h3>Payment Method: <%= rs1.getString(25) %> </h3></div>
...
<h3>Product Details</h3>
  ...
  <%
      ResultSet rs2 = st.executeQuery("Select * from cart inner join product on cart.product_id=product.id and cart.email='"+email+"' and cart.status='bill'");
      while(rs2.next())
      {
	  sno = sno+1;
  %>
